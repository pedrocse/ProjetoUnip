...
def gerar_relatorio():
    ...
    try:
        ...
        caminho_json = os.path.join(DIRETORIO_RELATORIOS, "relatorio_estudante.json")
        ...
        salvar_json(caminho_json, relatorio)

        with open(caminho_txt, "w", encoding="utf-8") as f_txt:
            f_txt.write("=" * 60 + "\n")
            f_txt.write("RELATÓRIO DE DESEMPENHO - APLICATIVO DE QUESTÕES\n")
            f_txt.write("=" * 60 + "\n\n")
            f_txt.write("DADOS DO ESTUDANTE:\n")
            f_txt.write("-" * 60 + "\n")
            f_txt.write(f"Nome: {st.session_state.nome_usuario}\n")
            f_txt.write(f"Registro Acadêmico: {st.session_state.registro_academico}\n")
            f_txt.write(f"Turma: {st.session_state.turma}\n")
            f_txt.write(f"Data: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n\n")
            f_txt.write("ESTATÍSTICAS:\n")
            f_txt.write("-" * 60 + "\n")
            f_txt.write(f"Total de Questões: {total_questoes}\n")
            f_txt.write(f"Questões Respondidas: {total_respondidas}\n")
            f_txt.write(f"Acertos: {st.session_state.total_acertos}\n")
            f_txt.write(f"Erros: {st.session_state.total_erros}\n")
            f_txt.write(f"Aproveitamento: {aproveitamento:.1f}%\n")
            f_txt.write(f"Tempo utilizado: {calcular_tempo_utilizado()}\n\n")
            f_txt.write("DETALHES DAS QUESTÕES:\n")
            f_txt.write("-" * 60 + "\n")
